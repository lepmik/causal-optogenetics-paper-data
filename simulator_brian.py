...
#            print('Rank {}'.format(nest.Rank()), *val)
    ...
    def set_neurons(self):
        ...
        self.nodes = brian2.NeuronGroup(
            self.p['N_neurons'],
            model=eqs,
            threshold=self.p['V_th'],
            reset=self.p['V_reset'],
        ...
        d = self.p['J_ex'] * self.p['tau_syn_ex'] * abs(self.p['E_L'] - self.p['E_ex'])
        self.p['J_in'] = - self.p['g'] * d / n

    def set_connections(self):
        # define synapses
        self.syn_ex = brian2.Synapses(
            self.nodes_ex,
            self.nodes,
            model='w:siemens',
            on_pre='ge+=w',
        ...
        self.vprint(
            ...
            self.p['C_ex'], 'and inhibitory neurons J = ', self.p['J_in'], 'C = ',
            self.p['C_in'])
        
        for j in tqdm(range(self.p['N_neurons'])):
        ...
        J_ex_j = np.clip(
            np.random.lognormal(
                ...
                size=len(self.syn_ex.w)),
            self.p['J_low'],
            self.p['J_high'])
        self.syn_ex.w = J_ex_j * nS
...
#    data_path, param_module = sys.argv[1:]
    ...
    data_path = '/home/jovyan/work/instrumentalVariable/data_sim/'
